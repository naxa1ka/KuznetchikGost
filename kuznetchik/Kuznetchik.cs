...
public class Kuznetchik
{
    ...
    private readonly byte[] _constR = { 148, 32, 133, 16, 194, 192, 1, 251, 1, 192, 194, 16, 133, 32, 148, 1 };
    ...
    {
        return a.Select(x => (byte)_sblock.ToList().IndexOf(x)).ToArray();
    }

    //Сложение в поле F(2) по модулю x^8 + x^7 + x^6 + x + 1
    private byte MultiplyGalua(byte a, byte b)
    {
        byte result = 0x00;
        while (b != 0x00)
        {
    ...
    //преобразование R
    public byte[] R(byte[] a)
    {
        byte c = 0x00;
        for (int i = 0; i < 16; i++)
        {
            c ^= MultiplyGalua(_constR[i], a[i]);
        }
